...
function entryHref(base, entry) {
  if (entry.kind === "category") {
    return base + "servers/?category=" + encodeURIComponent(entry.name);
  }
  var skillTags = {
    Skill: 1,
    Cursor: 1,
  ...
  if (skillTags[entry.name]) {
    return base + "skills/?tag=" + encodeURIComponent(entry.name);
  }
  return base + "servers/?tag=" + encodeURIComponent(entry.name);
}
