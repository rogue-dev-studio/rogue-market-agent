...
(function () {
  ...
  function findProduct(id) {
    var list = (window.RogueCatalog && RogueCatalog.assets) || [];
  ...
  function resolveImage(item) {
    if (window.RogueCards && RogueCards.itemImage) return RogueCards.itemImage(item, "assets");
  ...
  function platformsCategoryFromCatalog() {
    var fromCatalog =
      window.RogueCatalog && RogueCatalog.platformsCategoryLabel
        ? String(RogueCatalog.platformsCategoryLabel).trim()
        : "";
    var tree =
      (window.RogueCatalog && RogueCatalog.assetTaxonomyTree) || [];
    var labels = (window.RogueCatalog && RogueCatalog.platformLabels) || [];
    for (var t = 0; t < tree.length; t++) {
      var kids = tree[t].children || [];
      var hasPlatformChild = kids.some(function (child) {
        return labels.some(function (label) {
          return (
            String(child.label || "").toLowerCase() ===
            String(label || "").toLowerCase()
          );
        });
      });
      if (hasPlatformChild) return String(tree[t].label || "").trim();
    }
    if (fromCatalog) {
      var cats =
      ...
      for (var i = 0; i < cats.length; i++) {
        if (
          String(cats[i] || "").toLowerCase() === fromCatalog.toLowerCase()
        ) {
          return cats[i];
        }
      }
    ...
    (item.tags || []).forEach(function (tag) {
      String(tag)
        ...
        .forEach(function (part) {
          ...
          var platformCat = cat || platformsCategoryFromCatalog();
          if (platform && key === platform.toLowerCase()) {
            href = searchHref(
              "tag=" + encodeURIComponent(platformCat + "::" + platform)
            );
          } else if (platform && genreSet[key]) {
            href = searchHref(
              "tag=" +
                encodeURIComponent(platformCat + "::" + platform + "›" + part)
            );
            ...
            if (contentCat) {
              ...
              href = searchHref(
                "tag=" + encodeURIComponent(cat + "::" + part)
              );
            } else if (platform) {
              href = searchHref(
                "tag=" +
  ...
  function licenseCellHtml(item) {
    if (window.RogueLicense && typeof RogueLicense.classifyFromItem === "function") {
      var classified = RogueLicense.classifyFromItem(item, "assets");
  ...
  function showError(msg) {
    ...
    if (loading) {
      ...
      loading.textContent = msg || "Asset not found.";
    }
  ...
  function applyProductRating(item) {
    ...
    var countMode =
      window.RogueCards && typeof RogueCards.isCountOnlyRating === "function"
        ? RogueCards.isCountOnlyRating(item, "assets")
        : false;
    ...
    if (countMode) {
      var count =
        window.RogueCards && typeof RogueCards.engagementCount === "function"
          ? RogueCards.engagementCount(item, "assets")
  ...
  function render() {
    ...
    if (!item) {
      if (!(window.RogueCatalog && RogueCatalog.assetsLoaded)) return;
    ...
    document.title = item.name + " — Asset - Rogue Asset Store";
  ...
  document.addEventListener("rogue-catalog:assets-loaded", render);
  render();
})();
