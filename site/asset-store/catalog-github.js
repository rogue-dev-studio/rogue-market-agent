...
(function () {
  ...
  var TOPIC_META = {
    "rogue-asset-skills": 1,
    "rogue-asset-mcp": 1,
    "rogue-assets-skills": 1,
    "rogue-assets-mcp": 1,
  ...
  function searchTopics(topics) {
    ...
    return Promise.all(list.map(searchTopic)).then(function (parts) {
      var seen = {};
      var out = [];
      parts.forEach(function (items) {
        (items || []).forEach(function (repo) {
          var id = String((repo && repo.full_name) || (repo && repo.id) || "").toLowerCase();
  ...
  function load() {
    var skillTopics = catalog.topics.skills;
    var serverTopics = catalog.topics.servers;

    return Promise.all([searchTopics(skillTopics), searchTopics(serverTopics)]).then(function (parts) {
